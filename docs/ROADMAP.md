...
Replace duplicated Web preview planner state with the real shared planner/domain core, share routing/elevation/search providers, expose the same waypoint editing and undo/redo behavior on Web, and isolate only platform-specific capabilities such as background GPS and native offline regions.
